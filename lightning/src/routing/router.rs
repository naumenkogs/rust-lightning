...
impl PaymentPath {
	...
	fn update_value_and_recompute_fees(&mut self, value_msat: u64) {
		...
		for i in 0..self.hops.len() - 1 {
			let mut next_hop_use_fee_msat = self.hops.get(i + 1).unwrap().hop_use_fee_msat;
			let mut cur_hop = self.hops.get_mut(i).unwrap();
			let transferred_amount_msat = cur_hop.next_hops_fee_msat + value_msat + next_hop_use_fee_msat;
			...
			if let Some(extra_fees_msat) = cur_hop.htlc_minimum_msat.checked_sub(transferred_amount_msat) {
				// If this is triggered, next_hops_fee_msat gets outdated.
				next_hop_use_fee_msat += extra_fees_msat;
			}
			cur_hop.route_hop.fee_msat = next_hop_use_fee_msat;
		}
...
mod tests {
	...
	fn htlc_minimum_overpay_test() {
		...
		let route = get_route(&our_id, &net_graph_msg_handler.network_graph.read().unwrap(), &nodes[2], None, &Vec::new(), 30_000, 42, Arc::clone(&logger)).unwrap();
